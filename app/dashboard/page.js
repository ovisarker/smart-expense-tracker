...
export default function Dashboard(){
  ...
  async function invite(){
    ...
    if(!email) return;
    const res = await fetch("/api/household/invite",{ method:"POST", body: JSON.stringify({ email }) });
  ...
  async function saveBudget(){
    ...
    if(v==null) return;
    const r = await fetch("/api/users/budget",{ method:"POST", body: JSON.stringify({ monthlyBudget:Number(v) }) });
